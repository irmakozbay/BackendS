...
public class ChatSessionManager {
    ...
    private String inferCategory(String title, String criteriaJson, List<ChatMessage> messages) {
        ...
        if (combined.contains("uçak") || combined.contains("uçuş") || combined.contains("bilet") || combined.contains("flight") || combined.contains("havayolu") || combined.contains("havalimanı") || combined.contains("havaalanı") || combined.contains("gidiş") || combined.contains("dönüş")) {
            return "Flight";
        }
        if (combined.contains("otel") || combined.contains("hotel") || combined.contains("konaklama") || combined.contains("oda") || combined.contains("resort") || combined.contains("pansiyon")) {
            return "Hotel";
        }
        if (combined.contains("transfer") || combined.contains("servis") || combined.contains("taksi") || combined.contains("shuttle")) {
            return "Transfer";
        }
        return "General SOP";
    }
    ...
    private SessionSummaryResponse mapStateToSummary(SessionState s) {
        ...
        if (combined.contains("uçak") || combined.contains("uçuş") || combined.contains("bilet") || combined.contains("flight") || combined.contains("havayolu") || combined.contains("havalimanı") || combined.contains("havaalanı") || combined.contains("gidiş") || combined.contains("dönüş")) {
            cat = "Flight";
        } else if (combined.contains("otel") || combined.contains("hotel") || combined.contains("konaklama") || combined.contains("oda") || combined.contains("resort") || combined.contains("pansiyon")) {
            cat = "Hotel";
        } else if (combined.contains("transfer") || combined.contains("servis") || combined.contains("taksi") || combined.contains("shuttle")) {
            cat = "Transfer";
        }
        if (s.getMode() != null) {
            String mLower = s.getMode().toLowerCase();
            if ("hotel".equals(mLower) || "hotels".equals(mLower)) cat = "Hotel";
            else if ("flight".equals(mLower) || "flights".equals(mLower)) cat = "Flight";
            else if ("transfer".equals(mLower)) cat = "Transfer";
        }
    ...
    public List<SessionSummaryResponse> searchSessionsForUser(Long userId, String query) {
        ...
        return list.stream().filter(s -> {
            ...
            boolean titleMatch = s.getTitle() != null && (s.getTitle().toLowerCase().contains(rawQuery) || s.getTitle().toLowerCase().contains(cleanQuery));
            boolean categoryMatch = s.getCategory() != null && s.getCategory().toLowerCase().contains(rawQuery);
            boolean snippetMatch = s.getSnippet() != null && (s.getSnippet().toLowerCase().contains(rawQuery) || s.getSnippet().toLowerCase().contains(cleanQuery));
            
            boolean messageMatch = false;
            SessionState state = sessions.get(s.getId());
            if (state != null) {
                messageMatch = state.getMessages().stream().anyMatch(m -> {
                    if (m.getText() == null) return false;
